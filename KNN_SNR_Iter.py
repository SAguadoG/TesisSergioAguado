...
for noise_db in noise_levels_db:
    ...
    X_test_SNR_frequency = scaler_frequency.transform(X_test_SNR[:, 15:35])
    ...
    # Buscar el mejor valor de k usando GridSearchCV
    param_grid = {'n_neighbors': range(1, 15)}  # Ampliamos el rango de k
    grid_search = GridSearchCV(KNeighborsClassifier(), param_grid, cv=5, scoring='accuracy') # Usamos 5-fold CV
    grid_search.fit(X_train_normalized, y_train)

    best_k = grid_search.best_params_['n_neighbors']
    best_model = grid_search.best_estimator_
    best_y_pred = best_model.predict(X_test_SNR_normalized)
    accuracy = grid_search.best_score_

    # Calcular métricas
    print('Best k is', best_k)
    report = classification_report(y_test_SNR, best_y_pred, output_dict=True, zero_division=0)
    ...
    cm = confusion_matrix(y_test_SNR, best_y_pred)
...
plt.title('Rendimiento del Modelo vs. Nivel de Ruido para KNN')
plt.legend()
plt.grid(True)
