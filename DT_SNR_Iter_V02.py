...
# CARACTERÍSRTICAS DE LA ENVOLVENTE.
...
# UNIMOS TODOS LAS CARACTERÍSTICAS
...
X_train_clean, X_test_clean, y_train, y_test = train_test_split(
    original_signals, original_labels, train_size=0.8, random_state=42, stratify=original_labels)
...
print("\nIniciando el entrenamiento del modelo Decision Tree en datos con ruido aleatorio...")
...
print(f"Tiempo de computación del Entrenamiento + procesamiento: {train_time:.2f} segundos")
...
for noise_db in noise_levels_db:
    ...
    metricas_time_end = time.time()
    metricas_time = metricas_time_end - metricas_time_start
    end_time = time.time()
    iteration_time = end_time - start_time_eval
    times.append(iteration_time)

    print(f"Tiempo de computación Metricas {metricas_time:.2f} segundos")
    print(f"Tiempo de computación total para {noise_db} dB: {iteration_time:.2f} segundos")
...
plt.ylabel('Métricas')
plt.title('Rendimiento de Decision Tree vs. Nivel de Ruido')
plt.legend()
plt.grid(True)
