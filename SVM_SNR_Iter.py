...
for noise_db in noise_levels_db:
    ...
    # Crear el modelo SVM con parámetros estándar
    svm = SVC(C=100.0, kernel='rbf', gamma='scale', class_weight=None) # Parámetros por defecto
    ...
    print(f"Class Weight: {svm.class_weight}")

    # Entrenar el modelo
    svm.fit(X_train_normalized, y_train)

    # Predecir sobre el conjunto de prueba ruidoso
    y_pred = svm.predict(X_test_SNR_normalized)
    ...
    # Calcular métricas
    report = classification_report(y_test_SNR, y_pred, output_dict=True, zero_division=0)
