...
# CARACTERÍSRTICAS DE LA DERIVADA.
...
# UNIMOS TODAS LAS CARACTERISTICAS EN UN SOLO VECTOR.
...
# SE NORMALIZAN LOS DATOS, DA MEJORES RESULTADOS.
...
# Definir el modelo con la sugerencia de Keras para Input layer, Cuando lo hice con el modelo que me dijo la
# bibliografía de tensorflow, me saltó una alarma en la consola de que se recomentaba el input como se pone a
# continuación e iba mejor.

model = Sequential([
    Input(shape=(37,)),  # Usando Input layer como se sugiere
    ...
    Dense(10, activation='sigmoid')  # Capa de salida (10 clases)
])
...
# Añadir Early Stopping, esto lo que hace es si el valor deja de mejorar, en el epoch y se hace estable,
# guarda el valor y para el entramiento.
...
noise_levels_db = range(40, -1, -1)  # De 40 dB a 0 dB (inclusive), no quería añadir SNR a 0 dB, pero me equivoqué.
...
for noise_db in noise_levels_db:
    print(f"Evaluando con nivel de ruido: {noise_db} dB")
    #Hago controles de tiempo durante algunos sectores.
